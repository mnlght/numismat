...
<html class="no-js" lang="">
    <head>
        ...
        <title>Поиск по параметрам | Нумизматика</title> 
		...
		<section class="page-content">
			<div class="container">
	            <div class="row">
					<div class="col-md-12">
						<ul class="page-menu">
							<li><a href="main.php">Дом /</a></li>
							<li class="active"><a href="№">Поиск по параметрам</a></li>
						</ul>
					</div>
				</div>
				<div class="row">
	          ...
	          <?php include_once( __DIR__ . '/scripts/cat.php' ); ?>
						...
						<div class="accordion_one">
							...
							<div id="divone" class="collapse">
								<div class="filter-menu">
									<ul> 
										<li><input type="checkbox" name="material[]" value="'Ал.бронза'" id="check1" /><label for="check1" style="font-size:18px;">Ал.бронза</label></li>
										<li><input type="checkbox" name="material[]" value="'Алюминий'" id="check2"/><label for="check2" style="font-size:18px;">Алюминий</label></li>
										<li><input type="checkbox" name="material[]" value="'Биметалл'" id="check3"/><label for="check3" style="font-size:18px;">Биметалл</label></li>
										<li><input type="checkbox" name="material[]" value="'Бронза'" id="check4"/><label for="check4" style="font-size:18px;">Бронза</label></li>
										<li><input type="checkbox" name="material[]" value="'Железо'" id="check5"/><label for="check5" style="font-size:18px;">Железо</label></li>
										<li><input type="checkbox" name="material[]" value="'Золото'" id="check6"/><label for="check6" style="font-size:18px;">Золото</label></li>
										<li><input type="checkbox" name="material[]" value="'Латунь'" id="check7"/><label for="check7" style="font-size:18px;">Латунь</label></li>
										<li><input type="checkbox" name="material[]" value="'Медно-ник.'" id="check8"/><label for="check8" style="font-size:18px;">Медно-ник.</label></li>
										<li><input type="checkbox" name="material[]" value="'Медь'" id="check9"/><label for="check9" style="font-size:18px;">Медь</label></li>
										<li><input type="checkbox" name="material[]" value="'Мельхиор'" id="check10"/><label for="check10" style="font-size:18px;">Мельхиор</label></li>
										<li><input type="checkbox" name="material[]" value="'Никелевая бронза'" id="check11"/><label for="check11" style="font-size:18px;">Никелевая бронза</label></li>
										<li><input type="checkbox" name="material[]" value="'Плакированная сталь'" id="check12"/><label for="check12" style="font-size:18px;">Плакированная сталь</label></li>
					...
					<div class="col-md-9">
																				<?php
																				$valutnik=$_GET["valutnik"];
																				$query_valutnik=" AND valuta = '$valutnik'";
																				if (!empty($_GET["material"]))
																				{
																					$material = $_GET["material"];
																					$yy = implode('&material[]=',$material);
																					$check_brand = implode(', ',$_GET["material"]);
																				}
																				else
																					{
																						$check_brand = 'Любой металл';
																					}
																				$start_price = (int)$_GET["start_price"];
																				$end_price = (int)$_GET["end_price"];

																				if (($check_brand!='Любой металл') AND !empty($end_price))
																				{
																					if (!empty($check_brand)) $query_brand = "material IN ($check_brand)";
																					if (!empty($end_price)) $query_price = " AND price BETWEEN $start_price AND $end_price";
																				}else {
																					 	$query_brand = "";
																						$query_price = "price BETWEEN $start_price AND $end_price";
																				}
																				?>
						<!-- START PRODUCT-AREA -->
								...
								<div class="col-xs-12">
									...
									<div class="product-menu">
										<div class="product-title">
											<h3 class="title-group-3 gfont-1"><?php echo "$check_brand "; echo " от $start_price $valutnik"; echo " до $end_price $valutnik";?></h3>
										</div>
									</div>
									...
									<div class="product">
										<div class="tab-content">
											...
											<div role="tabpanel" class="tab-pane fade in active" id="display-1-1">
												<div class="row">
													<div class="listview">
														...
														error_reporting( E_ERROR );
															...
															$count = mysql_query("SELECT COUNT(*) FROM table_products WHERE $query_brand $query_price $query_valutnik",$link);
														...
														<?php
																				...
																				$result = mysql_query("SELECT * FROM table_products WHERE $query_brand $query_price $query_valutnik ORDER BY products_id DESC $qury_start_num",$link);
																						...
																						do
																						{
																							...
																							<div class="col-md-3 col-sm-4 col-xs-12">
																								...
																								<div class="product-img">
																									<a href="#" name="1" class="modal-view" data-toggle="modal" data-target="#'.$row["products_id"].'" title="Просмотр">
																							...
																							<div class="col-md-9 col-sm-8 col-xs-12">	
																								<div class="product-description">
																									<h5><a href="#" name="1" class="modal-view" data-toggle="modal" data-target="#'.$row["products_id"].'" title="Просмотр">'.$row["title"].'</a></h5>
																									<div class="price-box">
												...
												<?php
																  error_reporting( E_ERROR );
																	if ($page != 1) { $pstr_prev = '<li><a class="pstr-prev" href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page - 1).'">&lt;</a></li>';}
																	if ($page != $total) $pstr_next = '<li><a class="pstr_next" href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page + 1).'">&gt;</a></li>';

																	//Формируем ссылки со страницами
																	if($page - 5 > 0) $page5left = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page - 5).'">'.($page - 5).'</a></li>';
																	if($page - 4 > 0) $page4left = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page - 4).'">'.($page - 4).'</a></li>';
																	if($page - 3 > 0) $page3left = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page - 3).'">'.($page - 3).'</a></li>';
																	if($page - 2 > 0) $page2left = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page - 2).'">'.($page - 2).'</a></li>';
																	if($page - 1 > 0) $page1left = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page - 1).'">'.($page - 1).'</a></li>';

																	if($page + 5 <= $total) $page5right = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page + 5).'">'.($page + 5).'</a></li>';
																	if($page + 4 <= $total) $page4right = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page + 4).'">'.($page + 4).'</a></li>';
																	if($page + 3 <= $total) $page3right = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page + 3).'">'.($page + 3).'</a></li>';
																	if($page + 2 <= $total) $page2right = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page + 2).'">'.($page + 2).'</a></li>';
																	if($page + 1 <= $total) $page1right = '<li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.($page + 1).'">'.($page + 1).'</a></li>';


																	if ($page+5 < $total)
																	{
																		$strtotal = '<li><p class="nav-point">...</p></li><li><a href="search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page='.$total.'">'.$total.'</a></li>';
																														...
																														<div class="row">
																															<div class="col-xs-5">
																																<div class="pagination">
																																	<ul>
																																	';
																																	echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li class='active'><a  href='search_filter.php?valutnik='.$valutnik.'&start_price='.$start_price.'&end_price='.$end_price.'&material[]='.$yy.'&page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
																																	echo'
																																	</ul>
																																</div>
											...
											<div role="tabpanel" class="tab-pane fade " id="display-1-2">
												<div class="row">
													...
													<?php
																				...
																				$query = mysql_query("SET NAMES utf8");
																				$result = mysql_query("SELECT * FROM table_products WHERE $query_brand $query_price $query_valutnik ORDER BY products_id DESC",$link);
