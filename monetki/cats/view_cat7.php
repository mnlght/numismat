...
<html class="no-js" lang="">
    ...
    <body>
    	<?php
						    ...
						    if( (!isset($_SESSION['login'])) || (!isset($_SESSION['id'])) || ($_SESSION['id'] != 8) ){header("Location: login.php");
						    }else{
								?>
		<!-- HEADER-AREA START -->
					...
					<!-- Start logo & Search Box -->
						<div class="col-md-12">
							<div class="logo" style="width:100%;text-align: center;padding-top:0; ">
								<a href="cat7.php" title="Главная" style="font-size:80px;color:#26acce;width:100%;font-weight:900;">Нумизмат.</a>
							</div>
						</div>
					<!-- End logo & Search Box -->
				</div> 
						...
						<div class="col-lg-12">
							<div class="mainmenu">
								<nav style="text-align: center;">
									<ul>
										<li><a href="cat7.php" style="font-size:23px;">Вы допущены к категории Великобритания!</a></li>
									</ul>
								</nav>
							</div>
						</div>
	                                ...
	                                <ul>
	                                    ...
	                                    <li class="arrow-plus">
	                                    		...
	                                    		<?php
																					...
																					?>
	                                        <a href="cat7.php">Великобритания (<?=$res;?>) </a>
	                                        <!--  MEGA MENU START -->
	                                        <li>
																						     <?php
																									...
																									$result_set = mysql_query("SELECT * FROM table_products WHERE brand='Англия до 1837 года'",$link);
																										...
																										$row=mysql_fetch_array($result);
																											echo'
																											<li><a href="view_cat7.php?cat='.$row["brand"].'&type='.$row["type"].'">'.$row["brand"].' ('.$resl.')</a></li>
																											 ' ;
																						     ?>
																					</li>
																					<li>
								...
								<div class="col-xs-12">
									...
									<div class="product-filter">
										...
										<div class="sort">
											...
											<option value="view_cat7.php?cat=<?=$cat;?>&type=<?=$type;?>"><?php echo $sort_name; ?></option>
											<option value="view_cat7.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=name" >Название</option>
											<option value="view_cat7.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=price-asc">Цена по возрастанию</option>
											<option value="view_cat7.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=price-desc">Цена по убыванию</option>
											<option value="view_cat7.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=data">Дата создания</option>
											<option value="view_cat7.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=nominal">Номинал</option>
											</select>	
										</div>
										<div class="limit">
											<label>Вид:</label>
									...
									<div class="product">
										<div class="tab-content">
											...
											<div role="tabpanel" class="tab-pane fade in active" id="display-1-1">
												...
												<?php
																  error_reporting( E_ERROR );
																	if ($page != 1) { $pstr_prev = '<li><a class="pstr-prev" href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page - 1).'">&lt;</a></li>';}
																	if ($page != $total) $pstr_next = '<li><a class="pstr_next" href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page + 1).'">&gt;</a></li>';

																	//Формируем ссылки со страницами
																	if($page - 5 > 0) $page5left = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page - 5).'">'.($page - 5).'</a></li>';
																	if($page - 4 > 0) $page4left = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page - 4).'">'.($page - 4).'</a></li>';
																	if($page - 3 > 0) $page3left = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page - 3).'">'.($page - 3).'</a></li>';
																	if($page - 2 > 0) $page2left = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page - 2).'">'.($page - 2).'</a></li>';
																	if($page - 1 > 0) $page1left = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page - 1).'">'.($page - 1).'</a></li>';

																	if($page + 5 <= $total) $page5right = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page + 5).'">'.($page + 5).'</a></li>';
																	if($page + 4 <= $total) $page4right = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page + 4).'">'.($page + 4).'</a></li>';
																	if($page + 3 <= $total) $page3right = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page + 3).'">'.($page + 3).'</a></li>';
																	if($page + 2 <= $total) $page2right = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page + 2).'">'.($page + 2).'</a></li>';
																	if($page + 1 <= $total) $page1right = '<li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.($page + 1).'">'.($page + 1).'</a></li>';


																	if ($page+5 < $total)
																	{
																		$strtotal = '<li><p class="nav-point">...</p></li><li><a href="view_cat7.php?cat='.$cat.'&type='.$type.'&page='.$total.'">'.$total.'</a></li>';
																														...
																														<div class="row">
																															<div class="col-xs-5">
																																<div class="pagination">
																																	<ul>
																																	';
																																	echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li class='active'><a  href='view_cat7.php?cat='.$cat.'&type='.$type.'&page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
																																	echo'
																																	</ul>
																																</div>
