	            ...
	            <div class="row">
					<div class="col-md-12">
						<ul class="page-menu">
							...
							<li><a href="view_cat.php?type=<?=$type;?>"><?=$type;?></a></li>
							<li class="active"><a href="view_cat.php?cat=<?=$cat;?>&type=<?=$type;?>"><?=$cat;?></a></li>
						</ul>
					</div>
				</div>
				<div class="row">
								...
								<div class="col-xs-12">
									...
									<div class="product-filter">
										<div class="sort">
											...
											<option value="view_cat.php?cat=<?=$cat;?>&type=<?=$type;?>"><?php echo $sort_name ?></option>
											<option value="view_cat.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=name">Название</option>
											<option value="view_cat.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=price-asc">Цена по возрастанию</option>
											<option value="view_cat.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=price-desc">Цена по убыванию</option>
											<option value="view_cat.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=data">Дата создания</option>
											<option value="view_cat.php?cat=<?=$cat;?>&type=<?=$type;?>&sort=nominal">Номинал</option>
											</select>	
										</div>
										<div class="limit">
											<label>Вид:</label>
													...
													<div class="row">
														<div class="col-xs-5">
															<div class="pagination">
																<ul>
																';
																echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li class='active'><a  href='view_cat.php?cat='.$cat.'&type='.$type.'&page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
																echo'
																</ul>
															</div>
...
{ 
		...
		<!-- FOOTER-AREA END -->	
				<script type="text/javascript"> 
					<?php
					$identify = $_SESSION['id'];
					$pricepr = mysql_query("SELECT price FROM users WHERE id=$identify ",$link);
					$pepa = mysql_fetch_array($pricepr);
					?>
					var a = "<?=$pepa[0];?>";
					var elems = document.getElementsByClassName('price-box');
					var elems1 = document.getElementsByClassName('s-price-box');
					if (a != 1){
				...
				<script type="text/javascript">
					var id = "<?=$identify;?>";
					var elems2 = document.getElementsByClassName('product-button-3');
					if (id != 1){
